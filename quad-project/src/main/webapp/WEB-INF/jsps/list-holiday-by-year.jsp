...
<div style="border: solid; margin-top: 50px;">
		...
		<form action="yearsonly" style="background-color: #01BAEF;">
			<label style="margin-left: 120px;" for="cars">Filter - Year:</label>			
			<select id="yearOfList" name="year" onchange="changeFunc()" style="margin-right: 30px;">
				...
				<c:forEach items="${holidayDTOsDates}" var="holidayDateInYear">				
					<option value="${holidayDateInYear}" ${holidayDateInYear==selectedYear ?"selected":"" }>${holidayDateInYear}</option>
				</c:forEach>				
			</select> 
			<i class="fa fa-filter"></i><input style="margin-left: 10px;  background-color: #01BAEF; border: none; color: white;"  onclick="myFunction()" type="submit" value="Filter">
		</form>
		...
		<table class=" table" style="padding: 30px;">
			<thead class="thead-dark" style="border: none;">
				<tr
					style="color: white; border: none; border-color: rgb(35, 35, 45);">
